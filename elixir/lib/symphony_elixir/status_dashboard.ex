defmodule SymphonyElixir.StatusDashboard do
  ...
  defp humanize_claude_event(:session_initialized, _message, payload) do
    model = map_value(payload, ["model", :model])

    if is_binary(model) do
      "session initialized (model: #{model})"
    else
      "session initialized"
    end
  end

  defp humanize_claude_event(:assistant_message, message, payload) do
    # Extract text snippet from the assistant message
    # Try multiple paths: the summarized message wraps payload, or the unwrapped payload
    text =
      map_path(message, ["payload", "message", "content"]) ||
        map_path(payload, ["message", "content"]) ||
        map_path(message, ["message", "content"])
    ...
    if is_binary(snippet) and snippet != "" do
      "assistant: #{inline_text(snippet)}"
    else
      "assistant message received"
    end
  end

  defp humanize_claude_event(:stream_event, _message, _payload), do: "streaming..."

  defp humanize_claude_event(:turn_completed, message, payload) do
    # duration_ms comes from the details map (set by app_server result event)
    duration_ms =
      map_path(message, ["details", "duration_ms"]) ||
        map_value(payload, ["duration_ms", :duration_ms])

    # cost comes from the top-level metadata (set by app_server handle_result_event)
    cost =
      map_value(message, ["total_cost_usd", :total_cost_usd])

    # usage comes from the top-level metadata or inside the payload
    usage =
    ...
    parts =
      if is_number(duration_ms) do
        parts ++ ["#{Float.round(duration_ms / 1000, 1)}s"]
      else
        parts
      end
  ...
  defp humanize_claude_event(:turn_failed, message, payload) do
    error =
      map_path(message, ["details", "result"]) ||
        map_path(message, [:details, :result]) ||
        map_path(payload, ["params", "error", "message"]) ||
        map_path(payload, [:params, :error, :message])

    if is_binary(error), do: "turn failed: #{error}", else: "turn failed"
  end

  defp humanize_claude_event(:other_message, _message, _payload), do: nil
