defmodule SymphonyElixir.Linear.Client do
  ...
  def fetch_candidate_issues do
    ...
    cond do
      ...
      true ->
        effective_states = effective_poll_states(tracker.active_states, filters.states)
        label_filter = label_filter(filters.labels)

        with {:ok, assignee_filter} <- routing_assignee_filter(filters.assignee),
             {:ok, issues} <- do_fetch_by_states(project_slug, effective_states, assignee_filter) do
          {:ok, apply_label_filter(issues, label_filter)}
        end
    end
  end
  ...
  @doc false
  @spec apply_label_filter_for_test([Issue.t()], [String.t()]) :: [Issue.t()]
  def apply_label_filter_for_test(issues, labels) when is_list(issues) and is_list(labels) do
    apply_label_filter(issues, label_filter(labels))
  end

  @doc false
  @spec effective_poll_states_for_test([String.t()], [String.t()]) :: [String.t()]
  def effective_poll_states_for_test(active_states, filter_states)
      when is_list(active_states) and is_list(filter_states) do
    effective_poll_states(active_states, filter_states)
  end
  ...
  defp routing_assignee_filter(filters_assignee \\ nil) do
    assignee = filters_assignee || Config.settings!().tracker.assignee

    case assignee do
      nil -> {:ok, nil}
      value -> build_assignee_filter(value)
    end
  end

  defp effective_poll_states(active_states, []), do: active_states

  defp effective_poll_states(active_states, filter_states) when is_list(filter_states) do
    active_set = MapSet.new(active_states)

    filter_states
    |> Enum.filter(&MapSet.member?(active_set, &1))
    |> case do
      [] -> active_states
      narrowed -> narrowed
    end
  ...
  defp label_filter(labels) when is_list(labels), do: MapSet.new(Enum.map(labels, &String.downcase/1))

  defp apply_label_filter(issues, nil), do: issues

  defp apply_label_filter(issues, label_set) when is_struct(label_set, MapSet) do
    Enum.filter(issues, fn %Issue{labels: labels} ->
      Enum.any?(labels, &MapSet.member?(label_set, &1))
    end)
  end
