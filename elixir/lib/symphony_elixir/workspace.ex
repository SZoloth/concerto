defmodule SymphonyElixir.Workspace do
  ...
  defp run_hook(command, workspace, issue_context, hook_name, nil) do
    ...
    env = issue_env_vars(issue_context)
  ...
  defp run_hook(command, workspace, issue_context, hook_name, worker_host) when is_binary(worker_host) do
    timeout_ms = Config.settings!().hooks.timeout_ms
    export_lines = issue_export_lines(issue_context)

    Logger.info("Running workspace hook hook=#{hook_name} #{issue_log_context(issue_context)} workspace=#{workspace} worker_host=#{worker_host}")

    script = export_lines <> "cd #{shell_escape(workspace)} && #{command}"
  ...
  defp issue_env_vars(issue_context) do
    [
      {"CONCERTO_ISSUE_ID", issue_context.issue_id},
      {"CONCERTO_ISSUE_IDENTIFIER", issue_context.issue_identifier},
      {"CONCERTO_ISSUE_TITLE", issue_context.title},
      {"CONCERTO_ISSUE_STATE", issue_context.state}
    ]
    |> Enum.map(fn {key, value} -> {key, to_string(value || "")} end)
  end

  defp issue_export_lines(issue_context) do
    issue_context
    |> issue_env_vars()
