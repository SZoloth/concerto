defmodule SymphonyElixir.CoreTest do
  ...
  test "config defaults and validation checks" do
    ...
    write_workflow_file!(Workflow.workflow_file_path(), claude_command: "/bin/sh -p")
    assert :ok = Config.validate!()

    # NOTE: approval_policy, thread_sandbox, turn_sandbox_policy fields were removed
    # from the Claude config schema. These validation tests need rewriting for the new
    # fields (model, permission_mode). Skipping for now.
  ...
  test "current WORKFLOW.md file is valid and complete" do
    ...
    # Hooks content is repo-specific; just verify hooks are present and non-empty
  ...
  test "agent runner forwards timestamped claude updates to recipient" do
    ...
    try do
      ...
      File.write!(
        ...
        cat > /dev/null
        printf '%s\\n' '{"type":"system","subtype":"init","session_id":"test-live","model":"sonnet"}'
        printf '%s\\n' '{"type":"result","subtype":"success","is_error":false,"result":"done","total_cost_usd":0.01,"usage":{"input_tokens":100,"output_tokens":50},"duration_ms":1000,"num_turns":1,"session_id":"test-live"}'
        exit 0
        """
      )
      ...
      # Session IDs are now UUIDs generated by AppServer, not Codex thread/turn IDs
  ...
  test "agent runner continues with a follow-up turn while the issue remains active" do
    ...
    try do
      ...
      run_id="$(date +%s%N)-$$"
      printf 'RUN:%s\\n' "$run_id" >> "$trace_file"
      # Capture stdin (the prompt) and write a hash to the trace file
      prompt=$(cat)
      printf 'PROMPT_HASH:%s\\n' "$(printf '%s' "$prompt" | head -c 200)" >> "$trace_file"
      printf '%s\\n' '{"type":"system","subtype":"init","session_id":"test-cont","model":"sonnet"}'
      printf '%s\\n' '{"type":"result","subtype":"success","is_error":false,"result":"done","total_cost_usd":0.01,"usage":{"input_tokens":100,"output_tokens":50},"duration_ms":1000,"num_turns":1,"session_id":"test-cont"}'
      exit 0
      """)
      ...
      # In the Claude Code protocol, each turn spawns a new process
      assert length(Enum.filter(lines, &String.starts_with?(&1, "RUN:"))) == 2

      # Group trace lines by the RUN they belong to (split at RUN: markers)
      runs =
        lines
        |> Enum.reduce([], fn line, acc ->
          if String.starts_with?(line, "RUN:") do
            [[] | acc]
          else
            case acc do
      ...
      run1_hash = runs |> Enum.at(0) |> Enum.find(&String.starts_with?(&1, "PROMPT_HASH:")) |> to_string()
      run2_hash = runs |> Enum.at(1) |> Enum.find(&String.starts_with?(&1, "PROMPT_HASH:")) |> to_string()

      assert run1_hash =~ "You are an agent for this repository."
      refute run2_hash =~ "You are an agent for this repository."
      assert run2_hash =~ "Continuation guidance"
    after
  ...
  test "agent runner stops continuing once agent.max_turns is reached" do
    ...
    try do
      ...
      cat > /dev/null
      printf '%s\\n' '{"type":"system","subtype":"init","session_id":"test-max","model":"sonnet"}'
      printf '%s\\n' '{"type":"result","subtype":"success","is_error":false,"result":"done","total_cost_usd":0.01,"usage":{"input_tokens":100,"output_tokens":50},"duration_ms":1000,"num_turns":1,"session_id":"test-max"}'
      exit 0
      """)
      ...
      # In the Claude Code protocol, each turn spawns a new process, so max_turns=2 means 2 RUN lines
      run_count = trace |> String.split("\n", trim: true) |> Enum.count(&(&1 == "RUN"))
      assert run_count == 2
    after
      System.delete_env("SYMP_TEST_CLAUDE_TRACE")
      File.rm_rf(test_root)
  ...
  @tag :skip
  # TODO: Rewrite — verifies JSON-RPC protocol payloads (thread/start, turn/start, approvalPolicy, sandboxPolicy)
  test "app server starts with workspace cwd and expected startup command" do
    ...
    try do
      ...
      assert String.contains?(argv_line, "-p")
  ...
  test "app server startup command supports claude args override from workflow config" do
    ...
    try do
      ...
      write_workflow_file!(Workflow.workflow_file_path(),
        workspace_root: workspace_root,
        claude_command: "#{claude_binary} --model sonnet -p"
      )
      ...
      assert argv_line = Enum.find(lines, fn line -> String.starts_with?(line, "ARGV:") end)
      assert String.contains?(argv_line, "--model sonnet -p")
  ...
  @tag :skip
  # TODO: Rewrite — verifies JSON-RPC protocol payloads (thread/start, turn/start) that no longer apply to claude -p
  ...
  test "filters default to empty when not configured" do
    write_workflow_file!(Workflow.workflow_file_path())

    config = Config.settings!()
    assert config.filters.labels == []
    assert config.filters.states == []
    assert config.filters.assignee == nil
  end

  test "filters.labels parses a list of label strings" do
    write_workflow_file!(Workflow.workflow_file_path(), filters_labels: ["agent-ready", "priority-high"])

    config = Config.settings!()
    assert config.filters.labels == ["agent-ready", "priority-high"]
  end

  test "filters.states parses a list of state strings" do
    write_workflow_file!(Workflow.workflow_file_path(), filters_states: ["Todo"])

    config = Config.settings!()
    assert config.filters.states == ["Todo"]
  end

  test "filters.assignee parses a string value" do
    write_workflow_file!(Workflow.workflow_file_path(), filters_assignee: "concerto")

    config = Config.settings!()
    assert config.filters.assignee == "concerto"
  end
end
