defmodule SymphonyElixir.WorkspaceAndConfigTest do
  ...
  test "hooks receive CONCERTO_* env vars with issue metadata" do
    test_root =
      Path.join(
        System.tmp_dir!(),
        "symphony-elixir-workspace-env-#{System.unique_integer([:positive])}"
      )

    try do
      workspace_root = Path.join(test_root, "workspaces")
      env_dump = Path.join(test_root, "env.log")

      File.mkdir_p!(workspace_root)

      write_workflow_file!(Workflow.workflow_file_path(),
        workspace_root: workspace_root,
        hook_after_create: "env | grep CONCERTO_ | sort > \"#{env_dump}\""
      )

      issue = %{id: "uuid-123", identifier: "MT-99", title: "Fix the widget", state: "In Progress"}
      assert {:ok, _workspace} = Workspace.create_for_issue(issue)

      env_content = File.read!(env_dump)
      assert env_content =~ "CONCERTO_ISSUE_ID=uuid-123"
      assert env_content =~ "CONCERTO_ISSUE_IDENTIFIER=MT-99"
      assert env_content =~ "CONCERTO_ISSUE_TITLE=Fix the widget"
      assert env_content =~ "CONCERTO_ISSUE_STATE=In Progress"
    after
      File.rm_rf(test_root)
    end
  end

  test "hooks receive CONCERTO_* env vars with string identifier (minimal metadata)" do
    test_root =
      Path.join(
        System.tmp_dir!(),
        "symphony-elixir-workspace-env-minimal-#{System.unique_integer([:positive])}"
      )
    ...
    try do
      ...
      write_workflow_file!(Workflow.workflow_file_path(),
        ...
        hook_after_create:
          ~s(printf 'id=%s\\nident=%s\\ntitle=%s\\nstate=%s\\n' ) <>
            ~s("$CONCERTO_ISSUE_ID" "$CONCERTO_ISSUE_IDENTIFIER" ) <>
            ~s("$CONCERTO_ISSUE_TITLE" "$CONCERTO_ISSUE_STATE" > "#{env_dump}")
      )

      assert {:ok, _workspace} = Workspace.create_for_issue("MT-BARE")

      env_content = File.read!(env_dump)
      assert env_content =~ "id=\n"
      assert env_content =~ "ident=MT-BARE"
      assert env_content =~ "title=\n"
      assert env_content =~ "state=\n"
    after
      File.rm_rf(test_root)
    end
  end
  ...
  test "config reads defaults for optional settings" do
    ...
    # NOTE: approval_policy, thread_sandbox, turn_sandbox_policy, read_timeout_ms
    # were removed from the Claude config schema. Asserting new fields instead.
    ...
    write_workflow_file!(Workflow.workflow_file_path(), claude_command: "claude -p --model sonnet")
    assert Config.settings!().claude.command == "claude -p --model sonnet"
    ...
    # NOTE: approval_policy, thread_sandbox, turn_sandbox_policy removed from Claude schema.
    # These sandbox policy assertions need rewriting for the new config structure.
    ...
    # NOTE: read_timeout_ms was removed from the Claude config schema.
    ...
    # NOTE: approval_policy, thread_sandbox, turn_sandbox_policy removed from Claude schema.
    # Validation tests for these fields need rewriting for model/permission_mode.

    write_workflow_file!(Workflow.workflow_file_path(), claude_command: "claude -p")
    assert Config.settings!().claude.command == "claude -p"
  end
  ...
  # TODO: Sandbox policy tests removed — turn_sandbox_policy, resolve_turn_sandbox_policy,
  # resolve_runtime_turn_sandbox_policy, and claude_runtime_settings were all removed from
  # the Schema and Config modules. These tests need rewriting for the claude -p protocol.
  ...
  # runtime sandbox policy resolution test removed — references removed Schema functions
  ...
  test "label filter keeps issues that have any matching label" do
    issues = [
      %Issue{id: "1", identifier: "MT-1", labels: ["agent-ready", "backend"]},
      %Issue{id: "2", identifier: "MT-2", labels: ["frontend"]},
      %Issue{id: "3", identifier: "MT-3", labels: []}
    ]

    result = Client.apply_label_filter_for_test(issues, ["agent-ready"])

    assert Enum.map(result, & &1.identifier) == ["MT-1"]
  end

  test "label filter returns all issues when label list is empty" do
    issues = [
      %Issue{id: "1", identifier: "MT-1", labels: ["agent-ready"]},
      %Issue{id: "2", identifier: "MT-2", labels: []}
    ]

    result = Client.apply_label_filter_for_test(issues, [])

    assert Enum.map(result, & &1.identifier) == ["MT-1", "MT-2"]
  end

  test "label filter matches case-insensitively" do
    issues = [
      %Issue{id: "1", identifier: "MT-1", labels: ["agent-ready"]},
      %Issue{id: "2", identifier: "MT-2", labels: ["frontend"]}
    ]
  ...
  test "label filter includes issue matching any of multiple labels" do
    issues = [
      %Issue{id: "1", identifier: "MT-1", labels: ["agent-ready"]},
      %Issue{id: "2", identifier: "MT-2", labels: ["priority-high"]},
      %Issue{id: "3", identifier: "MT-3", labels: ["other"]}
    ]

    result = Client.apply_label_filter_for_test(issues, ["agent-ready", "priority-high"])

    assert Enum.map(result, & &1.identifier) == ["MT-1", "MT-2"]
  end

  test "effective poll states narrows to filter states when all are in active states" do
    active = ["Todo", "In Progress", "Rework"]
    filter = ["Todo"]

    result = Client.effective_poll_states_for_test(active, filter)

    assert result == ["Todo"]
  end

  test "effective poll states ignores filter states not in active states" do
  ...
  test "effective poll states returns active states when filter is empty" do
    active = ["Todo", "In Progress"]

    result = Client.effective_poll_states_for_test(active, [])

    assert result == ["Todo", "In Progress"]
  end

  test "effective poll states falls back to active states when no filter states overlap" do
    active = ["Todo", "In Progress"]
    filter = ["Backlog", "Cancelled"]
