defmodule SymphonyElixir.AppServerTest do
  ...
  @tag :skip
  # TODO: Rewrite — references removed claude_turn_sandbox_policy config and JSON-RPC turn/start protocol
  test "app server passes explicit turn sandbox policies through unchanged" do
    ...
    try do
      ...
      trace_file = Path.join(test_root, "claude-supported-turn-policies.trace")
      ...
      #!/bin/sh
      trace_file="${SYMP_TEST_CLAUDE_TRACE:-/tmp/claude-supported-turn-policies.trace}"
      count=0
      ...
      Enum.each(policy_cases, fn configured_policy ->
        ...
        write_workflow_file!(Workflow.workflow_file_path(),
          workspace_root: workspace_root,
          claude_command: "#{claude_binary} -p"
        )
  ...
  @tag :skip
  # TODO: Codex-specific — turn/input_required is a JSON-RPC method with no Claude Code equivalent
  ...
  @tag :skip
  # TODO: Codex-specific — approval requests handled via --permission-mode in Claude Code
  ...
  @tag :skip
  # TODO: Codex-specific — approval policy handled via --permission-mode in Claude Code
  ...
  @tag :skip
  # TODO: Codex-specific — tool approval handled via --permission-mode in Claude Code
  ...
  @tag :skip
  # TODO: Codex-specific — tool user input prompts are a JSON-RPC feature with no Claude Code equivalent
  ...
  @tag :skip
  # TODO: Codex-specific — dynamic tool calls handled via MCP in Claude Code
  ...
  test "app server buffers partial JSON lines until newline terminator" do
    ...
    try do
      ...
      File.write!(claude_binary, """
      #!/bin/sh
      # Read the prompt from stdin (piped from prompt file)
      cat > /dev/null
      # Emit a system init line with padding that exceeds the port line buffer
      padding=$(printf '%*s' 1100000 '' | tr ' ' a)
      printf '{"type":"system","subtype":"init","session_id":"test-91","model":"sonnet","padding":"%s"}\\n' "$padding"
      printf '%s\\n' '{"type":"result","subtype":"success","is_error":false,"result":"done","total_cost_usd":0.01,"usage":{"input_tokens":100,"output_tokens":50},"duration_ms":1000,"num_turns":1,"session_id":"test-91"}'
      exit 0
      """)
  ...
  test "app server captures claude side output and logs it through Logger" do
    ...
    try do
      ...
      # Read the prompt from stdin (piped from prompt file)
      cat > /dev/null
      printf '%s\\n' '{"type":"system","subtype":"init","session_id":"test-92","model":"sonnet"}'
      printf '%s\\n' 'warning: this is stderr noise' >&2
      printf '%s\\n' '{"type":"result","subtype":"success","is_error":false,"result":"done","total_cost_usd":0.01,"usage":{"input_tokens":100,"output_tokens":50},"duration_ms":1000,"num_turns":1,"session_id":"test-92"}'
      exit 0
      """)
      ...
      assert log =~ "Claude stream output: warning: this is stderr noise"
    after
      File.rm_rf(test_root)
    end
  end
  ...
  test "app server launches over ssh for remote workers" do
    ...
    try do
      ...
      printf 'ARGV:%s\\n' "$*" >> "$trace_file"
      # No stdin to read — the prompt file redirect is embedded in the remote command string
      printf '%s\\n' '{"type":"system","subtype":"init","session_id":"test-remote","model":"sonnet"}'
      printf '%s\\n' '{"type":"result","subtype":"success","is_error":false,"result":"done","total_cost_usd":0.01,"usage":{"input_tokens":100,"output_tokens":50},"duration_ms":1000,"num_turns":1,"session_id":"test-remote"}'
      exit 0
      """)
      ...
      write_workflow_file!(Workflow.workflow_file_path(),
        ...
        claude_command: "fake-remote-claude -p"
      )
      ...
      assert argv_line =~ "fake-remote-claude -p"
    after
      File.rm_rf(test_root)
    end
  end
