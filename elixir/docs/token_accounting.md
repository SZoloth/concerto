...
This document explains how Claude Code reports token usage through the app-server protocol and how Concerto should account for it.

It is based on the current Claude Code source in `codex-rs`, especially:
...
Claude Code defines `TokenUsageInfo` like this:
...
This is the most important accounting rule in the Claude Code source.

## Event Types

### `codex/event/token_count`

Claude Code core emits token count events containing `TokenUsageInfo`.
...
Concerto sees these events wrapped inside the app-server message stream.
...
Claude Code uses the word `usage` in multiple places.
...
Claude Code also has logic that can "fill to context window", which sets:
...
For Concerto, `model_context_window` should be displayed or logged separately from spend.

## Recommended Accounting Strategy For Concerto

Track usage per active Claude Code thread.
...
## What Concerto Should And Should Not Do
...
- Expect one thread to span multiple turns when Concerto reuses a live Claude Code thread.
...
## Practical Interpretation For Concerto Logs
...
Claude Code itself consistently prefers cumulative totals when it needs durable state:
...
That is a strong signal for Concerto:

- use absolute totals as the main accounting surface
- ignore last/delta values for totals

## Recommended Concerto Documentation Contract

If Concerto documents token reporting externally, the contract should be:

- Live token totals come from Claude Code thread-scoped cumulative usage.
- Incremental usage may also be emitted, but Concerto does not use it for totals.
